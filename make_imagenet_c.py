...
class DistortImageFolder(data.Dataset):
    def __init__(self, root, method, severity, split='train', transform=None, target_transform=None,
    ...
    def __getitem__(self, index):
        ...
        save_path = '../data/imagenet-c/' + self.method.__name__ + \
                    '/' +self.split + '/' + self.idx_to_class[target]
...
def save_distorted(method=gaussian_noise):
    ...
    for split in ['train', 'val']:
        distorted_dataset = DistortImageFolder(
            root=f"../data/imagenet/{split}/",
            split=split,
            method=method, severity=severity,
            transform=trn.Compose([trn.RandomResizedCrop(224)]))
        distorted_dataset_loader = torch.utils.data.DataLoader(
            distorted_dataset, batch_size=1000, shuffle=False, num_workers=32)
